...
void dp_find_collision_parallel(size_t hash_len, dp_hash_function_t h,
                                dp_random_byte_generator_t rnd,
  ...
  for (size_t i = 0; i < num_threads; ++i) {
    pthread_create(&(trail_threads[i]), NULL, trail_thread,
                   (void *)&trail_thread_data);
  }

  bool collision_found = false;
  while (!collision_found) {
    pthread_cond_wait(&(trail_thread_data.possible_collision_cv),
                      &(trail_thread_data.possible_collision_mutex));

    uint8_t y0_longer[hash_len], y0_smaller[hash_len];
    uint64_t length_diff;
    if (trail_thread_data.collision_trail_1->l >
        trail_thread_data.collision_trail_2->l) {
      ...
      length_diff = trail_thread_data.collision_trail_1->l -
                    trail_thread_data.collision_trail_2->l;
    } else {
      ...
      length_diff = trail_thread_data.collision_trail_2->l -
                    trail_thread_data.collision_trail_1->l;
    }
...
dp_trail_t *create_dp_trail(uint8_t const *y0, size_t y0_len, uint8_t const *dp,
                            size_t dp_len, uint64_t l) {
